...
| Privilege or local-IPC abuse during Windows export | Keep the main app unelevated; use a signed one-shot broker restricted to the exact current eligible profile, authenticate both IPC endpoints, bound the response, and never write export XML | Substitute caller/profile identities, replay responses, exceed bounds, cancel UAC, and inspect disk; the broker returns no credential and leaves no file |
